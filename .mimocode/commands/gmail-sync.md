---
description: Sync Application Status from Gmail — classify status-signal emails against tracked applications and write approved updates to the tracker and archive.
---
...
Confirm the Gmail MCP tools are available in this session. If not, tell the user to connect Gmail:

- On claude.ai: Settings → Connectors → Gmail.
- On MiMo Code: there is no equivalent in-TUI connector flow. Instead, configure the Gmail MCP server via `mimo mcp add` (guided local/remote MCP setup) per `.mimocode/mimocode.json`'s `mcp` config block, then run `mimo mcp auth <server-name>` from the shell to complete OAuth if the server needs it.

Stop after telling the user this - do not attempt this via the bash tool, IMAP, or any other channel.
