---
description: Generate a Job-Portal Search Skill for Your Local Market — investigate, scaffold, and live-test a new portal-search CLI skill.
---
...
- If `$ARGUMENTS` contains `--list`: use the glob tool with `.agents/skills/*/SKILL.md`, print a table of installed portal skills (name, market from the description, data source from `url-reference.md`), and stop.
...
Do reconnaissance before writing any code. Use the webfetch tool (or `curl` via the bash tool) on the portal:
...
1. Install dev types and typecheck (via the bash tool):
   ```bash
