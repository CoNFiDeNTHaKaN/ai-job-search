---
description: Push Ranked Jobs and Applications to a Notion Database — one-way, read-only presentation sync from the tracker and seen_jobs into Notion.
---
...
1. Check that Notion MCP tools are available in this session (tool names starting with `mcp__notion__` or similar, or whatever naming scheme this session's MCP integration uses). Determine this from the session's own tool list **only** - never by running shell commands like `mimo mcp list`, which would interrupt the user with a permission prompt before the graceful exit. If the tools are not available, stop and tell the user how to connect:
   > Notion MCP isn't connected. Add it to `.mimocode/mimocode.json`'s `mcp` block as a `type: "remote"` entry with `url: "https://mcp.notion.com/mcp"` (or run the guided `mimo mcp add` from the shell), then restart MiMo Code so the new server loads (servers added mid-session are only picked up on restart). Once loaded, run `mimo mcp auth notion` from the shell to complete the OAuth login — MiMo also auto-detects OAuth-required servers and may prompt inline on first tool use. Then re-run `/notion-sync`.
2. Verify the connection with one cheap call (e.g. a workspace search). An auth error → tell the user to re-authenticate via `mimo mcp auth notion` from the shell and stop. Never retry in a loop.
3. The Notion MCP server is interactively authenticated, so "connected but not authenticable right now" (expired OAuth, headless/CI context where the login flow cannot run) gets the same graceful exit as "not configured": state the reason in one line and stop. This includes the configured-but-unauthenticated state where the server exposes only its auth handshake and no data tools - **never initiate the OAuth flow from this command and never ask whether to authenticate now**; the one line points at `mimo mcp auth notion` and the command ends there. Authenticating is the user's move, made outside this command.
