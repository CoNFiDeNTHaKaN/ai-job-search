---
description: Drafter-Reviewer Job Application Workflow — evaluate fit, draft a tailored CV and cover letter, get them reviewed, and compile/verify the PDFs.
---
...
- When invoking the reviewer subagent, pass draft content **inline in the task message** rather than asking it to read files you already have in memory.
...
- If `$ARGUMENTS` looks like a URL, use the webfetch tool to retrieve the job posting content.
- **If the fetch returns HTTP 403, or the content is a login wall or an unrelated listing page, do not give up and do not draft from the title.** Follow the escalation order in `.claude/skills/job-application-assistant/09-web-research.md`: retry with browser headers via curl (the bash tool), then search for the employer's own careers posting. Most corporate and bank sites reject the default user agent while serving the page normally to a browser.
...
Write both files to disk using the write tool. Keep the exact text of both drafts in working memory — you will pass them inline to the reviewer subagent in Step 3 and revise them in Step 4 without re-reading.

---

## Step 3: REVIEWER - Research & Critique

Use the task tool to invoke the `reviewer` subagent (defined in `.mimocode/agents/reviewer.md`). That subagent already contains the full reviewer logic — the trust boundary, company research steps, the reference files it reads, the factual grounding audit, the requirement coverage check, and the Part A (structured edits) / Part B (narrative suggestions) output format. Do not repeat any of that logic here; just invoke it correctly.

Pass the subagent a task message containing, all inline (do not make the subagent read the draft files):
- The company name and role title extracted in Step 0
- The full job posting text
- The full CV draft text (the exact content just written to `cv/main_<company>_<role><CV_EXT>`)
- The full cover letter draft text (the exact content just written to `cover_letters/cover_<company>_<role><COVER_EXT>`)

The reviewer subagent has no write or edit tool access — it returns its Part A / Part B feedback as its final message; it never touches disk.
...
2. **Apply Part B (narrative suggestions)** using judgment. These need interpretation, not mechanical replacement. Walk through every Part B category the reviewer returned and address it:
   ...
   - **Company/department-specific angles:** weave the reviewer's research into the cover letter opening or motivation paragraph. Verify every company claim via the webfetch/websearch tools before including it — do not trust reviewer research at face value.
   ...
   Use the edit tool for targeted changes; only re-read a file if an edit fails because the surrounding text has shifted.
...
After all edits are applied, the two files on disk are the final drafts. This is a real two-phase flow — draft, then subagent critique, then drafter revision — never collapse Steps 2-4 into a single pass.
...
Use `<CV_COMPILE>` and `<COVER_COMPILE>` resolved in Step 2 (the active template's declared compile command, or the stock defaults below if no custom template is active), via the bash tool:
...
Do not proceed to Step 6 until both PDFs pass inspection. Pass/fail is exactly the checklist above: every box in 5b checked for both PDFs is a pass; any unchecked box means iterate in 5c and recompile before moving on.
...
**Availability check:** run `pdftotext -v` (via the bash tool). `pdftotext` (poppler) is an optional dependency, not part of TeX distributions. If it is missing, print a one-line warning that the mechanical parse check is skipped, do the keyword-coverage check (item 3 below) against your visual read of the PDF instead, and note the degraded mode in the Step 6 report. Same graceful-skip pattern as the salary lookup.
