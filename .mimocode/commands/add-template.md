---
description: Register a Custom CV or Cover Letter Template — store, verify, and activate a user-supplied CV or cover letter template for /apply.
---
...
Use the glob tool with `templates/**/TEMPLATE.md` to find registered templates. For each, read the manifest and print a table:
...
2. Use the glob tool with `templates/**/TEMPLATE.md` and find manifests whose parent folder name exactly matches `<name>`.
...
Ask the user directly in the conversation (skip anything already answered by `$ARGUMENTS`):
...
Write into it, using the write tool:
...
2. Compile with the declared compile command via the bash tool, substituting `_compile_test` for `<file>`:
   ```bash
...
4. On success, confirm a PDF was produced and read it via the read tool to check the layout renders sensibly (no overlapping text, fonts loaded, page count matches the declared page limit for the dummy content). Record any surprises in the manifest's "Known pitfalls".
...
Insert (or replace, if one exists) this block immediately after the file's H1 title, using the edit tool:
