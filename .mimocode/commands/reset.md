---
description: Reset Candidate Profile Data — destructively clear profile skill files and/or the documents folder, with a mandatory typed confirmation gate.
---
...
Use the glob tool to list all files present in `documents/cv/`, `documents/linkedin/`, `documents/diplomas/`, `documents/references/`, and `documents/applications/`. Present as:
...
For each non-empty document subfolder, delete all files within it using the bash tool's `rm`. Do not delete the folder itself, and do not delete `documents/README.md`.
